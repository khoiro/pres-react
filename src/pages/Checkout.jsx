...
export default function Checkout() {
  ...
  // Lokasi GPS (checkout_lat / checkout_lng): otomatis pakai plugin Capacitor
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("checkout_img", photoBlob, `keluar-${today}.jpeg`);
      formData.append("checkout_lat", currentLat);
      formData.append("checkout_lng", currentLng);
      formData.append("checkout_category", category);
      formData.append("unor_id", homeData.unor_id);
      formData.append("nip", homeData.nip);
      formData.append("checkout_lat_branch", branch.lat);
      formData.append("checkout_lng_branch", branch.lng);
      formData.append("checkout_branch", branch.branch_name);
      formData.append("unor", homeData.unor);
      formData.append("checkout_loc_note", locNote);
      formData.append("checkout_range_km", `${distanceLabel}`);
      ...
      Swal.fire({
        ...
        text:
          err.response?.data?.message ||
          "Terjadi kesalahan saat mengirim data checkout.",
      });
    } finally {
      setSubmitting(false);
    }
    ...
    return (
      <div className="card card-custom">
        <div className="card-body">
          <h3>Checkout Presensi</h3>

          <hr />

          <div className="alert alert-warning mb-0">
            Anda belum melakukan checkin hari ini, sehingga checkout belum dapat
            dilakukan.
          </div>
        </div>
      </div>
  ...
  if (attendance?.today?.has_checkout) {
    return (
      <div className="card card-custom">
        <div className="card-body">
          <h3>Checkout Presensi</h3>
          ...
          <div className="alert alert-success mb-0">
            Anda sudah melakukan checkout hari ini pukul{" "}
            {attendance.today.checkout?.slice(11, 19)}.
          </div>
        </div>
      </div>
    );
      ...
      <div className="card-body">
        ...
        <div className="row g-4">
          <div className="col-lg-6">
            <h6 className="mb-3">Ambil Foto Checkout</h6>
        ...
        <form onSubmit={handleSubmit}>
          <div className="row g-3 align-items-end">
            <div className="col-md-4">
              <label className="form-label">Kategori Checkout</label>
          ...
          >
            ...
            {submitting ? "Mengirim..." : "Checkout Sekarang"}
          </button>

          {!photoBlob && (
            <div className="form-text text-danger mt-2">
              Ambil foto terlebih dahulu sebelum checkout.
            </div>
          )}
        </form>
      </div>
